    ...
    while (getline(infile, line)) {
        ...
        if (line[0] == '#' && line.length() >= 6) {
            if (line.substr(0, 6) == "#Q = {") {
                ...
                for (auto ite = _state_set.begin(); ite != _state_set.end();
                     ite++) {
                    unordered_map<string, TransitionOutput> temp{};
                    _trans_fun.insert(
                        pair<string, unordered_map<string, TransitionOutput> >(
                            *ite, temp));
                }
            }
            ...
            if (line.substr(0, 6) == "#F = {") {
                ...
                while (1) {
                    ...
                    while (Q_is_legal(line[idx + count])) count++;
                    if (line[idx + count] == ',') {
                        if (_state_set.find(line.substr(idx, count)) !=
                            _state_set.end()) {
            ...
            if (_state_set.find(vec[0]) == _state_set.end() ||
                _state_set.find(vec[4]) == _state_set.end()) {
                init();
                infile.close();
                return false;
            }
...
void TM::step() {
    ...
    if (_trans_fun[_state].find(now_tapes) != _trans_fun[_state].end()) {
        ...
        bool matched = 0;
        int min_star_count = _tape_num + 1;
        for (auto ite = _trans_fun[_state].begin();
             ite != _trans_fun[_state].end(); ite++) {
            string pattern = ite->first;
            int star_num = 0;
            for (int i = 0; i < pattern.length(); i++) {
                if (pattern[i] == '*') star_num++;
            }
            if (match(now_tapes, pattern) && min_star_count > star_num) {
                output = ite->second;
                min_star_count = star_num;
                matched = 1;
            }
        }
        if (!matched) {
    ...
    _state = output.state;
    for (int i = 0; i < _tapes.size(); i++) {
        if (output.tape_characters[i] != '*')
            _tapes[i].set(output.tape_characters[i]);
